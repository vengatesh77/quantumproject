...
import { getDesignations, addDesignation, updateDesignation, deleteDesignation, getDepartments } from '../services/api';
...
const Designations = () => {
  ...
  const [formData, setFormData] = useState({
    title: '', department: '', minSalary: '', maxSalary: '', experience: '', status: 'Active'
  });
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast('Error loading data. Check API server.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (currentDesig) {
        await updateDesignation(currentDesig.id, { ...currentDesig, ...formData });
        toast('Designation updated successfully ✅');
      } else {
        const maxNum = designations.reduce((max, d) => {
          const n = parseInt((d.desigId || '').replace('DES', '')) || 0;
          return n > max ? n : max;
        }, 0);
        await addDesignation({ ...formData, desigId: `DES${String(maxNum + 1).padStart(3, '0')}` });
        toast('Designation added successfully ✅');
      }
      setShowModal(false);
      await loadData();
    } catch {
      toast('Error saving designation.', 'error');
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast('Error deleting designation.', 'error');
    }
  };
      ...
      <div className="section-header">
        ...
        <div className="section-actions">
          <div className="search-input-wrap hide-print">
            <Search size={16} />
            <input type="text" className="form-control" placeholder="Search designations..."
          ...
          {loading ? (
            ...
            <table className="table">
              ...
              <tbody>
                {filtered.map(des => (
                  <tr key={des.id}>
                    <td className="font-medium text-muted">{des.desigId}</td>
                    ...
                    <td>₹{Number(des.minSalary).toLocaleString()} – ₹{Number(des.maxSalary).toLocaleString()}</td>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan="7" className="empty-state"><Briefcase size={48}/><h3>No designations found</h3></td></tr>
                )}
              </tbody>
            </table>
          )}
            ...
            <form onSubmit={handleSave}>
              <div className="modal-body form-grid-2">
                ...
                <div className="form-group">
                  ...
                  <input type="number" className="form-control" value={formData.minSalary}
                    onChange={e => setFormData({...formData, minSalary: Number(e.target.value)})} required />
                </div>
                <div className="form-group">
                  <label className="form-label">Max Salary *</label>
                  <input type="number" className="form-control" value={formData.maxSalary}
                    onChange={e => setFormData({...formData, maxSalary: Number(e.target.value)})} required />
                </div>
                <div className="form-group">
                  <label className="form-label">Experience Required *</label>
                  <input type="text" className="form-control" value={formData.experience}
